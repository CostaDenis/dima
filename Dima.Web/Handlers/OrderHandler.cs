    ...
    public class OrderHandler(IHttpClientFactory httpClientFactory) : IOrderHandler
    {
        ...
        public async Task<Response<Order?>> CancelAsync(CancelOrderRequest request)
        {
            var result = await _client
                .PostAsJsonAsync($"v1/orders/{request.Id}/cancel", request);
        ...
        {
            var result = await _client
                .PostAsJsonAsync("v1/orders/", request);
        ...
        {
            var result = await _client
                .PostAsJsonAsync($"v1/orders/{request.Number}/pay", request);
        ...
        {
            var result = await _client
                .PostAsJsonAsync($"v1/orders/{request.Id}/refund", request);
